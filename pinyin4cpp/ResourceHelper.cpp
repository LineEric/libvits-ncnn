...
QHash<wstring, wstring> *ResourceHelper::loadProperties(wstring resourcePath)
{
    QHash<wstring, wstring> *hash=new  QHash<wstring, wstring>();
    ...
    while (!in.atEnd()) {
        wstring line = in.readLine();

        wstringList stringList=line.split(wchar_t ::Space);
